...
function FundTransfer() {
    ...
    const submitHandler = (data) => {
        ...
        data.sourceAccountNumber = parseInt(data?.sourceAccountNumber, 10);
                ...
                <Modal.Body>
                    <form onSubmit={handleSubmit(submitHandler)}>
                        <div className="form-group mb-3">
                            <label htmlFor="sourceAccountNumber" className="fw-semibold">Source Account Number <span className="text-danger">*</span></label>
                            <input type="text" inputMode="numeric" className="form-control form-control-sm" disabled id="sourceAccountNumber" {...register('sourceAccountNumber', { required: 'This field is required.', pattern: { value: /^\d+$/, message: 'Please enter valid account number.' } })} />
                            {errors?.sourceAccountNumber && <small className="text-danger">{errors?.sourceAccountNumber?.message}</small>}
                        </div>
                        <div className="form-group mb-3">
                            <label htmlFor="destinationAccountNumber" className="fw-semibold">Destination Account Number <span className="text-danger">*</span></label>
                            <input type="text" inputMode="numeric" className="form-control form-control-sm" id="destinationAccountNumber" {...register('destinationAccountNumber', { required: 'This field is required.', pattern: { value: /^\d+$/, message: 'Please enter valid account number.' } })} />
                            {errors?.destinationAccountNumber && <small className="text-danger">{errors?.destinationAccountNumber?.message}</small>}
                        </div>
                        <div className="form-group mb-3">
